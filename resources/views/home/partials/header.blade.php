        ...
        <nav id="navigation" class="navigation navigation-landscape">
            <div class="nav-header">
                ...
                <a class="nav-brand static-logo" href="{{route('home')}}"><img
                        src="{{$setting->icon ? asset('storage/'.$setting->icon):'/images/logo.png'}}" class="logo"
                        alt="" /></a>
                <a class="nav-brand fixed-logo" href="{{route('home')}}"><img
                        src="{{$setting->icon ? asset('storage/'.$setting->icon):'/images/logo2.png'}}" class="logo"
                        alt="" /></a>
                @else
                <a class="nav-brand" href="{{route('home')}}">
                    <img src="{{$setting->icon ? asset('storage/'.$setting->icon):'/images/logo.png'}}" class="logo"
                        alt="" />
                </a>
                @endif
                ...
                <div class="mobile_nav">
                    ...
                    @can('is_admin')
                    <ul>
                        <li><a href="{{route('admin.home')}}"><i class="fas fa-user-circle fa-lg"></i></a></li>
                        <li class="_my_prt_list"><a href="{{route('logout')}}">خروج</a></li>
                    </ul>
                    @endcan
                    @can('is_agent')
                    <ul>
                        <li><a href="{{route('agent.home')}}"><i class="fas fa-user-circle fa-lg"></i></a></li>
                        <li class="_my_prt_list"><a href="{{route('logout')}}">خروج</a></li>
                    </ul>
                    @endcan
                    @can('is_user')
                    <ul>
                        <li><a href="{{route('user.home')}}"><i class="fas fa-user-circle fa-lg"></i></a></li>
                        <!-- <li class="_my_prt_list"><a href="{{route('logout')}}">خروج</a></li> -->
                        <li class="_my_prt_list"><a href="{{route('user.show')}}"><span
            ...
            <div class="nav-menus-wrapper" style="transition-property: none;">
                <ul class="nav-menu">
                    ...
                    <li @class(['active'=>url()->current()==route('properties.list')])><a href="#">املاک<span
                                class="submenu-indicator"></span></a>
                    ...
                    <li @class(['active'=>url()->current()==route('blog.index')])><a
                            href={{route('blog.index')}}>اخبار</a></li>

                    <li @class(['active'=>url()->current()==route('article.index')])><a
                            href={{route('article.index')}}>مقالات</a></li>
                ...
                <ul class="nav-menu nav-menu-social align-to-right">
                    ...
                    <li class="_my_prt_list"><a href="{{route('user.show')}}" style="color:#00c746"><span
                                class="cart-btn_counter color-bg" id="count">{{$wishlist->count()}}</span>علاقه مندی
                            ها</a>
