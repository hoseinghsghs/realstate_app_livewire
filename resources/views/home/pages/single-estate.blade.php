                        ...
                        <div class="property_info_detail_wrap_first">
                            <div class="pr-price-into">
                                ...
                                <span><i class="lni-map-marker"></i>
                                    {{ $property->province . ' ، ' . $property->city . ' ، ' . $property->district }}</span>
                            </div>
                        </div>
                        ...
                        <div class="property_detail_section">
                            <div class="prt-sect-pric">
                                @if ($property->tr_type === 'رهن و اجاره')
                                    <p>مبلغ رهن:<span class="pr-2 text-muted">{{ number_format($property->rahn) }}
                                            تومان</span></p>
                                    <p>مبلغ اجاره:<span class="pr-2 text-muted">{{ number_format($property->rent) }}
                                            تومان</span></p>
                                @else
                                    <p>قیمت:<span class="pr-2 text-muted">{{ number_format($property->bidprice) }}
                                            تومان</span></p>
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="_prtis_list mb-4">
                        ...
                        <div class="_prtis_list_body">
                            <ul class="deatil_features">
                                @isset($property->code)<li><strong>{{ $property->code }}</strong>شناسه ملک</li>
                                @endisset
                                @isset($property->floor)<li><strong>{{ $property->floor }}</strong>تعداد طبقات
                                </li>@endisset
                                @isset($property->year)<li><strong>{{ $property->year }}</strong>مدت ساخت</li>
                                @endisset
                                @isset($property->area)<li><strong>{{ $property->area }}</strong>مساحت زمین ملک
                                </li>@endisset
                                @if ($property->tr_type === 'رهن و اجاره')
                                    @isset($property->people_number)<li>
                                            <strong>{{ $property->people_number }}</strong>تعداد نفرات
                                    </li>@endisset
                                @else
                                    @isset($property->loan)<li><strong>{{ $property->loan }}</strong>وام بانکی
                                    </li>@endisset
                                    @if ($property->loanamount)
                                        <li><strong>{{ number_format($property->loanamount) }} تومان</strong>مبلغ وام</li>
                                    @endif
                                    @if ($property->meter_price)
                                        <li><strong>{{ number_format($property->meter_price) }} تومان</strong>قیمت هر متر
                                        مربع</li>@endisset
                                @endif
                                @isset($property->doc)<li><strong>{{ $property->doc }}</strong>نوع سند
                                </li>@endisset
                                @isset($property->dimension)<li>
                                        <strong>{{ $property->dimension }}</strong>ابعاد
                                </li>@endisset
                                @isset($property->view)<li><strong>{{ $property->view }}</strong>نما</li>
                                @endisset
                                @isset($property->door)<li><strong>{{ $property->door }}</strong>درب ورودی
                                </li>@endisset
                                @isset($property->screen)<li><strong>{{ $property->screen }}</strong>پرده
                                </li>@endisset
                                @isset($property->cover)<li><strong>{{ $property->cover }}</strong>کفپوش
                                </li>@endisset
                                @isset($property->phone_line)<li><strong>{{ $property->phone_line }}</strong>خط
                                        تلفن</li>
                                @endisset
                                @isset($property->cool)<li><strong>{{ $property->cool }}</strong>سیستم
                                    برودتی</li>@endisset
                                @isset($property->heat)<li><strong>{{ $property->heat }}</strong>سیستم
                                    حرارتی</li>@endisset
                                @isset($property->cabinet)<li><strong>{{ $property->cabinet }}</strong>کابینت
                                </li>@endisset
                                @isset($property->collection)<li>
                                        <strong>{{ $property->collection }}</strong>تعداد واحدهای
                                        مجموعه
                                </li>@endisset
                        </ul>
                    </div>
